...
export default function Progress() {
        ...
        if (data) {
          const entries: TimelineEntry[] = data.map(log => {
            ...
            if (log.entry_type === 'meal') {
              const mealMacros = log.macros && typeof log.macros === 'object' ? log.macros as {
                protein?: number;
                carbs?: number;
                fat?: number;
              } : undefined;
  ...
  const handleDeleteAchievement = async () => {
    ...
    const {
      error
    } = await supabase.from('fasting_sessions').delete().eq('id', idToDelete);
    if (error) {
      toast({
        title: '❌ Erro',
        variant: 'destructive'
  ...
  const handleMealEditSubmit = async (data: {
    ...
    const {
      error
    } = await supabase.from('meal_logs').update(data).eq('id', editingMeal.id);
    if (error) {
      toast({
        title: '❌ Erro',
        variant: 'destructive'
  ...
  return <div className="min-h-[100dvh] bg-background relative">
        ...
        <div className="relative z-10">
          ...
          <main className="px-4 pt-4 space-y-4 pb-[20px]">
            ...
            <div className="grid grid-cols-4 gap-2">
              {loading ? Array.from({
              length: 4
            }).map((_, i) => <Skeleton key={i} className="h-20 rounded-2xl" />) : stats.map((stat, index) => <motion.div key={stat.label} initial={{
              opacity: 0,
              y: 20
            }} animate={{
              opacity: 1,
            ...
            <Tabs defaultValue="hoje" className="w-full">
              ...
              <TabsContent value="hoje" className="mt-4 space-y-4">
                ...
                <div className="space-y-2 pb-[20px]">
                  ...
                  {loading ? Array.from({
                  length: 3
                }).map((_, i) => <Skeleton key={i} className="h-20 rounded-xl" />) : sortedTimeline.length > 0 ? sortedTimeline.map((entry, index) => <motion.div key={entry.id} initial={{
                  opacity: 0,
                  y: 10
                }} animate={{
                  opacity: 1,
                  ...
                  delay: index * 0.03
                      ...
                      </motion.div>) : <div className="text-center py-8 rounded-xl bg-card border border-border">
                  ...
                  {loading ? <Skeleton className="h-28 rounded-xl" /> : <>
                      <div className="flex gap-1 overflow-x-auto scrollbar-hide pb-2">
                        {weeks.map((week, weekIndex) => <div key={weekIndex} className="flex flex-col gap-1">
                            {week.map((day, dayIndex) => <motion.div key={`${weekIndex}-${dayIndex}`} initial={{
                        scale: 0
                      }} animate={{
                        scale: 1
                      }} transition={{
                        delay: 0.2 + weekIndex * 0.01
                      }} className={cn("w-3.5 h-3.5 rounded-sm", day.intensity === 0 && "bg-muted", day.intensity === 1 && "bg-[#84cc16]/30", day.intensity === 2 && "bg-[#84cc16]/50", day.intensity === 3 && "bg-[#84cc16]/70", day.intensity === 4 && "bg-[#84cc16]")} title={`${day.date.toLocaleDateString('pt-BR')} - ${day.count} atividades`} />)}
                          </div>)}
                      </div>
      ...
      <MealEditDialog open={mealEditDialogOpen} onOpenChange={setMealEditDialogOpen} onSubmit={handleMealEditSubmit} initialData={{
