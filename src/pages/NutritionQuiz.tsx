...
export default function NutritionQuiz() {
  ...
  return <div className="min-h-[100dvh] bg-background relative overflow-hidden">
      ...
      {/* Header */}
      <header className="fixed top-0 left-0 right-0 z-50 bg-card/80 backdrop-blur-xl border-b border-border" style={{
      paddingTop: 'calc(2rem + env(safe-area-inset-top, 0px))'
    }}>
        <div className="max-w-2xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between mb-4">
            <Button variant="ghost" size="sm" onClick={handleBack} className="press-effect">
        ...
        <div className="max-w-lg mx-auto">
          <AnimatePresence mode="wait">
            ...
            duration: 0.3
                ...
                return <motion.button key={option} onClick={() => handleOptionSelect(option)} whileHover={{
                  scale: 1.01
                }} whileTap={{
                  scale: 0.99
                }} className={`w-full p-4 rounded-2xl border transition-all text-left press-effect ${isSelected ? 'border-primary bg-primary/10 shadow-violet' : 'border-border bg-card hover:border-primary/50 hover:bg-accent/30'}`}>
              ...
              y: 0
                    ...
                    <Button onClick={handleSubmit} disabled={!isComplete} className="w-full h-14 rounded-2xl gradient-primary text-white font-semibold text-lg press-effect shadow-violet">
