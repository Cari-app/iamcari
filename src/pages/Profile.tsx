...
import { User, Camera, Loader2, Clock, LogOut, Moon, Sun, Scale, Bell, HelpCircle, Shield, ChevronRight, Sparkles, Utensils, Timer } from 'lucide-react';
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchEssentialData = async () => {
      const [profileResult, fastingResult] = await Promise.all([supabase.from('profiles').select('full_name, nickname, avatar_url, weight, height, active_diet').eq('id', user.id).single(), supabase.from('fasting_sessions').select('start_time, end_time').eq('user_id', user.id).not('end_time', 'is', null)]);
      ...
      if (profileData) {
        ...
        if (profileData.active_diet) {
          supabase.from('diet_types').select('id, name, icon, short_description, color_theme').eq('id', profileData.active_diet).single().then(({
            data: dietData
          }) => {
  ...
  const handleSaveBodyStats = async () => {
    ...
    try {
      ...
      } = await supabase.from('assessments').select('gender, age, activity_level, goal_type, goal_speed, target_weight_kg').eq('user_id', user.id).order('created_at', {
        ascending: false
      }).limit(1).maybeSingle();
      const {
        ...
        <div className="relative z-10">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="relative flex-shrink-0">
                  ...
                  <div className="relative w-20 h-20 group">
                    {avatarUrl ? <img src={avatarUrl} alt="Avatar" className="w-20 h-20 rounded-2xl object-cover border-2 border-[#84cc16]/50" /> : <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-green-600 to-green-800 flex items-center justify-center border-2 border-[#84cc16]/50">
                ...
                <div className="flex-1 min-w-0 space-y-2">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {isAdmin && <Badge className="px-2 py-0.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white font-bold text-xs border-none">
                          <Shield className="h-3 w-3 mr-1" />
                          ADMIN
                        </Badge>}
                    </div>
                    {nickname && <p className="text-sm text-muted-foreground">
                        @{nickname}
                      </p>}
                  </div>
          ...
          <div className="px-4 pt-4">
            <div className="grid grid-cols-3 gap-3">
              ...
              <button onClick={() => navigate('/diets')} className="p-4 rounded-2xl bg-card border border-border flex flex-col items-center gap-2 press-effect hover:bg-muted/50 transition-colors">
                <div className="h-10 w-10 rounded-xl bg-[#84cc16]/20 flex items-center justify-center">
              ...
              <button onClick={() => navigate('/fasting-quiz')} className="p-4 rounded-2xl bg-card border border-border flex flex-col items-center gap-2 press-effect hover:bg-muted/50 transition-colors">
                <div className="h-10 w-10 rounded-xl bg-[#84cc16]/20 flex items-center justify-center">
          ...
          {activeDiet && <div className="px-4 pt-4">
              <div className="p-4 rounded-2xl bg-card border border-border">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => navigate('/diet-result')} className="h-7 text-xs">
                    Ver detalhes
          ...
          <main className="px-4 pt-4 space-y-4 pb-[20px]">
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-2xl bg-card border border-border overflow-hidden">
                <button onClick={handleEditPersonalData} className="w-full flex items-center justify-between p-4 press-effect hover:bg-muted/50 transition-colors border-b border-border">
                ...
                <button onClick={handleEditBodyStats} className="w-full flex items-center justify-between p-4 press-effect hover:bg-muted/50 transition-colors">
                  <div className="flex items-center gap-3">
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-2xl bg-card border border-border overflow-hidden">
                ...
                <div className="flex items-center justify-between p-4">
                  <div className="flex items-center gap-3">
                    {theme === 'dark' ? <Moon className="h-5 w-5 text-[#84cc16]" /> : <Sun className="h-5 w-5 text-yellow-500" />}
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-2xl bg-card border border-border overflow-hidden">
                <button onClick={() => navigate('/help')} className="w-full flex items-center justify-between p-4 press-effect hover:bg-muted/50 transition-colors border-b border-border">
                  <div className="flex items-center gap-3">
                ...
                <button onClick={() => navigate('/privacy')} className="w-full flex items-center justify-between p-4 press-effect hover:bg-muted/50 transition-colors">
                  <div className="flex items-center gap-3">
          ...
          <div className="mx-auto w-full max-w-lg bg-card border border-border rounded-t-3xl">
            ...
            <div className="px-4 pb-4 space-y-4">
              <div className="space-y-2">
                ...
                <Input id="weight" type="number" value={bodyWeight || ''} onChange={e => setBodyWeight(parseFloat(e.target.value) || 0)} placeholder="Ex: 70" className="bg-muted/50 border-border rounded-xl" />
              </div>

              <div className="space-y-2">
                <Label htmlFor="height">Altura (cm)</Label>
                <Input id="height" type="number" value={bodyHeight || ''} onChange={e => setBodyHeight(parseFloat(e.target.value) || 0)} placeholder="Ex: 170" className="bg-muted/50 border-border rounded-xl" />
              </div>
            </div>

            <DrawerFooter className="pt-2">
              <Button onClick={handleSaveBodyStats} disabled={savingBodyStats} className="w-full bg-[#84cc16] hover:bg-[#84cc16]/90 text-white">
          ...
          <div className="mx-auto w-full max-w-lg bg-card border border-border rounded-t-3xl">
            ...
            <div className="px-4 pb-4 space-y-4">
              <div className="space-y-2">
                ...
                <Input id="fullName" value={editFullName} onChange={e => setEditFullName(e.target.value)} placeholder="Seu nome" className="bg-muted/50 border-border rounded-xl" />
              </div>

              <div className="space-y-2">
                <Label htmlFor="nickname">Apelido (opcional)</Label>
                <Input id="nickname" value={editNickname} onChange={e => setEditNickname(e.target.value)} placeholder="Ex: joaosilva" className="bg-muted/50 border-border rounded-xl" />
            ...
            <DrawerFooter className="pt-2">
              <Button onClick={handleSavePersonalData} disabled={savingPersonalData} className="w-full bg-[#84cc16] hover:bg-[#84cc16]/90 text-white">
